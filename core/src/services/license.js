...
const { LICENSE_ENABLED, LICENSE_SECRET } = require('../config/license-config');
...
function aesEncrypt(plainText, secret) {
  const aesKey = crypto.scryptSync(secret, 'salt', 32);
...
function generateLicenseKey(machineId, secret = LICENSE_SECRET) {
  if (!secret) {
    throw new Error('FARM_LICENSE_SECRET is required when license validation is enabled');
  }

  const normalizedId = machineId.replace(/-/g, '').toUpperCase();
  const combined = normalizedId + secret;
  const hashInput = aesEncrypt(combined, secret);
...
async function verifyAndRun() {
  ...
  if (!LICENSE_SECRET) {
    console.error('[授权检查] FARM_LICENSE_SECRET 未配置');
    return false;
  }
